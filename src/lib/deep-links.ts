...
export function generateLinks(input: string): LinkData {
  ...
  if (hostname === "twitter.com" || hostname === "x.com") {
    ...
    if (
      firstSegment &&
      !["home", "explore", "notifications"].includes(firstSegment)
    ) {
      return {
        platform: "twitter",
  ...
  if (hostname === "linkedin.com") {
    // /in/username -> linkedin://profile/username
    ...
    // /company/companyname -> linkedin://company/companyname
    if (pathname.startsWith("/company/")) {
      const company = pathname.replace("/company/", "");
      return {
        platform: "linkedin",
        deepLink: `linkedin://company/${company}`,
        webLink: urlStr,
        originalUrl: urlStr,
      };
    }
  ...
  if (hostname === "pinterest.com" || hostname.endsWith(".pinterest.com")) {
    // pin/ID -> pinterest://pin/ID
    const pinMatch = pathname.match(/\/pin\/(\d+)/);
    if (pinMatch) {
      const pinId = pinMatch[1];
      return {
        platform: "pinterest",
        deepLink: `pinterest://pin/${pinId}`,
        webLink: urlStr,
        originalUrl: urlStr,
      };
    }
    // user profile? pathname is usually /username/
    // Hard to distinguish from other pages without API.
    // But if it's just /username, we can try
    const parts = pathname.split("/").filter(Boolean);
    if (parts.length === 1) {
      return {
        platform: "pinterest",
        deepLink: `pinterest://user/${parts[0]}`,
        webLink: urlStr,
        originalUrl: urlStr,
      };
    }
  ...
  if (hostname === "facebook.com") {
    // basic matching. facebook:// is tricky.
    // facebook://facewebmodal/f?href=URL usually works for universal
    return {
      platform: "facebook",
      deepLink: `fb://facewebmodal/f?href=${encodeURIComponent(urlStr)}`,
      webLink: urlStr,
      originalUrl: urlStr,
    };
  }
  ...
  if (hostname === "twitch.tv") {
    ...
    if (parts.length === 1) {
      return {
        platform: "twitch",
        deepLink: `twitch://stream/${parts[0]}`, // or twitch://open?stream=...
        webLink: urlStr,
        originalUrl: urlStr,
      };
    }
  ...
  if (hostname === "reddit.com" || hostname === "www.reddit.com") {
    // simple fallback to https which usually triggers app intent
    // or reddit://
    // Let's try to mimic the path structure for `reddit://`
    return {
      platform: "reddit",
      deepLink: `reddit://${hostname}${pathname}`, // Experimental scheme
      webLink: urlStr,
      originalUrl: urlStr,
    };
  }
